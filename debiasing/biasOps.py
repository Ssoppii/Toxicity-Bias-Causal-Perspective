...
def identify_bias_subspace(vocab, def_sets, subspace_dim, embedding_dim):
    ...
    for k, v in def_sets.items():
        # collect valid vectors for this set
        wSet = [vocab[w] for w in v if w in vocab]
        if len(wSet) == 0:
    ...
    for k, v in def_sets.items():
        if k not in means:
            continue  # skip sets we could not compute
        wSet = [vocab[w] for w in v if w in vocab]
        if len(wSet) == 0:
            continue
        set_vectors = np.array(wSet)
        ...
        matrix.append(diffs)
    if not matrix:
        raise ValueError("No valid defining vectors found to compute subspace.")
...
def equalize_and_soften(vocab, words, bias_subspace, embedding_dim, l=0.2, verbose=True):
    ...
    # Filter out words not in vocab and build tensor efficiently
    vec_list = [vocab[w] for w in words if w in vocab]
    np_vecs = np.array(vec_list, dtype=np.float32)
    Neutrals = torch.from_numpy(np_vecs).t()

    # Efficiently create tensor from numpy array
    np_vecs_all = np.array(vocabVectors, dtype=np.float32)
    Words = torch.from_numpy(np_vecs_all).t()
